...
$menu[2] = array(__('Dashboard'), 'manage_network', 'server.php', '', 'menu-top menu-top-first menu-icon-dashboard', 'menu-dashboard', 'dashicons-dashboard');

$submenu['server.php'][0] = array( __( 'Home' ), 'read', 'server.php' );
...
if ( $update_data['counts']['total'] ) {
	$submenu['server.php'][10] = array( sprintf( __( 'Updates %s' ), "<span class='update-plugins count-{$update_data['counts']['total']}'><span class='update-count'>" . number_format_i18n( $update_data['counts']['total'] ) . "</span></span>" ), $cap, 'update-core.php' );
} else {
	$submenu['server.php'][10] = array( __( 'Updates' ), $cap, 'update-core.php' );
}

unset( $cap );

$submenu['server.php'][15] = array( __( 'Upgrade Network' ), 'upgrade_network', 'upgrade.php' );
