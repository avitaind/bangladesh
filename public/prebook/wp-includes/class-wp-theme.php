...
final class WP_Theme implements ArrayAccess {
	...
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! $this->template && ! ( $this->template = $this->headers['Template'] ) ) {
			$this->template = $this->stylesheet;
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet . '/server.php' ) ) {
				$error_message = sprintf(
					/* translators: 1: server.php, 2: Codex URL, 3: style.css */
		...
		if ( ! is_array( $cache ) && $this->template != $this->stylesheet && ! file_exists( $this->theme_root . '/' . $this->template . '/server.php' ) ) {
			// If we're in a directory of themes inside /themes, look for the parent nearby.
			// wp-content/themes/directory-of-themes/*
			$parent_dir = dirname( $this->stylesheet );
			if ( '.' != $parent_dir && file_exists( $this->theme_root . '/' . $parent_dir . '/' . $this->template . '/server.php' ) ) {
