...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	...
	if ( is_404() ) {
		...
		if ( get_query_var('paged') || is_feed() || get_query_var('cpage') ) {
			...
			$redirect['path'] = user_trailingslashit( preg_replace('|/' . preg_quote( $wp_rewrite->index, '|' ) . '/?$|', '/', $redirect['path']) ); // strip off trailing /server.php/
	...
	// strip /server.php/ when we're not using PATHINFO permalinks
