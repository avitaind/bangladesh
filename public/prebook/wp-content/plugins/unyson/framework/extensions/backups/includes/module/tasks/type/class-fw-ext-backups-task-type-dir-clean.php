...
class FW_Ext_Backups_Task_Type_Dir_Clean extends FW_Ext_Backups_Task_Type {
	...
	public function execute(array $args, array $state = array()) {
		 ...
		 */
		{
			...
			if (!file_exists("$backups_dir/server.php")) {
				$contents = implode("\n", array(
					'<?php',
					'header(\'HTTP/1.0 403 Forbidden\');',
					'die(\'<h1>Forbidden</h1>\');'
				));
				if (@file_put_contents("$backups_dir/server.php", $contents) === false) {
					return new WP_Error(
						'index_create_fail', sprintf( __('Cannot create file: %s', 'fw'), "$backups_dir/server.php" )
					);
				}
			}
